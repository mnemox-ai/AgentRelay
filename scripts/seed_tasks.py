"""Generate 10 seed tasks as JSON for AgentRelay development and testing."""
...
SEED_TASKS = [
    # --- data_structuring (3) ---
    {
        ...
        "difficulty": "easy",
        "input_data": {
            "raw_text": "Apple Inc. reported Q4 2025 revenue of $95.4B, up 8% YoY. "
    ...
    },
    {
        ...
        "difficulty": "medium",
        "input_data": {
            "raw_text": "Product: Wireless Mouse X1\nPrice: $29.99\nRating: 4.5/5 (1,203 reviews)\n"
    ...
    },
    {
        ...
        "difficulty": "hard",
        "input_data": {
            "raw_text": "Clinical Trial NCT00001234: Phase 3, randomized, double-blind. "
    ...
    },
    {
        ...
        "difficulty": "medium",
        "input_data": {
            "query": "Compare the top 3 Python async web frameworks by GitHub stars, latest release date, and key features.",
    ...
    },
    {
        ...
        "difficulty": "hard",
        "input_data": {
            "query": "Summarize the current state of WebAssembly adoption in server-side applications.",
            "context": "Include runtime options (Wasmtime, Wasmer, WasmEdge), "
    ...
    },
    {
        ...
        "input_data": {
            "language": "python",
            "task": "Implement a simple LRU cache class with `get(key)` and `put(key, value)` methods. "
    ...
    },
    {
        ...
        "input_data": {
            "language": "python",
            "task": "Write an async rate limiter using the token bucket algorithm. "
            "It should support `async acquire()` that waits until a token is available, "
            "and `try_acquire()` that returns immediately.",
            "constraints": [
...
def main():
    output = {
        ...
        "count": len(SEED_TASKS),
        "tasks": SEED_TASKS,
    }

    output_path = "scripts/seed_tasks.json"
    with open(output_path, "w", encoding="utf-8") as f:
        json.dump(output, f, indent=2, ensure_ascii=False)

    print(f"Generated {len(SEED_TASKS)} seed tasks → {output_path}")
    ...
    for task_type, tasks in sorted(by_type.items()):
        ...
        print(f"  {task_type}: {len(tasks)} tasks ({', '.join(difficulties)})")
